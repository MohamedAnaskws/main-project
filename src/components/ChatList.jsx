...
const ChatList = () => {
  ...
  const getOtherUserFromConversation = useCallback((conversation) => {
    ...
    if (conversation.participants && conversation.participants.length > 0) {
      ...
      if (otherParticipant?.user_id) {
        ...
        return userMap[otherParticipant.user_id];
      }
    }
    
    // Then try to get from last_message sender
    if (conversation.last_message?.sender) {
      const sender = conversation.last_message.sender;
      // If sender is not the current user, return sender
  ...
  const handleArchive = async (e, conversationId) => {
    e.stopPropagation();
    if (
      window.confirm("Archive this chat? You can find it in Archived chats.")
    ) {
  ...
  const getUserDisplayName = (userData) => {
    ...
    return { fullName, userNo: null };
  };

  // Force re-render when conversations change
  useEffect(() => {
    if (conversations.length > 0 && users.length === 0 && !loadingUsers) {
      fetchUsers();
    }
  }, [conversations]);
