...
export default function AttendancePage() {
  ...
  const formatDateTime = (dt) =>
    dt ? dayjs(dt).format("MMM DD, HH:mm") : "--:--";
    ...
    async (userId, fromDate, toDate) => {
      ...
      try {
        ...
        const response = await axios.get(
          `${BASE}/api/attendance/attendance-logs-each-day`,
          {
            params: {
              user_id: userId,
              from_date: fromDate,
  ...
  const fetchAttendance = useCallback(
    async (date) => {
      try {
        ...
        const token = getToken();
        const response = await axios.get(
          `${BASE}/api/attendance/report/${dayjs(date).format("YYYY-MM-DD")}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "ngrok-skip-browser-warning": "true",
        ...
        const formatted = attendances.map((item) => ({
          ...
          workPercent: Math.min(
            100,
            ((item.total_work_minutes || 0) / 600) * 100,
          ),
  ...
  const chartData = data.map((u) => ({
    ...
    work: parseFloat(u.work),
    break: parseFloat(u.break),
  }));
  ...
  const WorkProgress = ({
    ...
    <Tooltip
      title={
        <div>
          <div>
            Check In: <strong>{formatTime(checkIn)}</strong>
          </div>
          <div>
            Check Out: <strong>{formatTime(checkOut)}</strong>
          </div>
          <div>
            Break: {breakMinutes} min ({breakTime}h)
          </div>
    ...
    >
      <div className="cursor-pointer">
        <div className="flex justify-between text-xs mb-1">
          <span>
            <LoginOutlined /> {formatTime(checkIn)}
          </span>
          <span>
            <LogoutOutlined /> {formatTime(checkOut)}
          </span>
          <span>Work: {work}h</span>
        </div>
        <div className="h-5 bg-gray-200 rounded-md overflow-hidden">
          <div
            className="h-full bg-emerald-500 flex items-center justify-center text-white text-xs"
            style={{ width: `${Math.min(100, workPercent)}%` }}
          >
        ...
        <div className="flex justify-between text-xs mt-1">
          ...
          <span className="text-amber-600">
            Break: {breakTime}h ({breakMinutes}m)
          </span>
        </div>
      </div>
    </Tooltip>
  ...
  const columns = [
      ...
      render: (_, r) => (
        <div className="flex items-center gap-2">
          <Avatar
            icon={<UserOutlined />}
            style={{
              backgroundColor: r.status === "Present" ? "#10b981" : "#f43f5e",
            }}
          />
          <div>
    ...
    },
    {
      ...
      align: "center",
      width: 100,
      sorter: (a, b) => parseFloat(a.work) - parseFloat(b.work),
      render: (_, r) => (
        <div>
          <span className="font-bold text-emerald-600 text-base">{r.work}</span>
    ...
    },
    {
      ...
      render: (_, r) => (
        <div>
          <span className="text-amber-600 font-bold">{r.break}h</span>
          <div className="text-xs">({r.totalBreakMinutes}m)</div>
        </div>
      ),
    },
    {
      ...
      >
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <Text type="secondary">Target: 10h/day</Text>
          </div>
          <DatePicker
            value={selectedDate}
        ...
        <Modal
          title={`Attendance Details: ${selectedEmployee?.name}`}
          open={modalVisible}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <Tag color="blue">
                          <CalendarOutlined /> {log.date} ({log.day_name})
                        </Tag>
                        ...
                        {log.summary?.overtime_hours > 0 && (
                          <Tag color="orange">
                            Overtime: +{log.summary.overtime_hours}h
                          </Tag>
                        )}
                      </div>
                      <div className="text-right">
                        <div className="font-bold text-emerald-600 text-lg">
                          {(log.summary?.total_worked_hours || 0).toFixed(2)}h
                        </div>
                    ...
                    <div className="space-y-3">
                      {sessions.map((session, sIdx) => {
                        const sessionDate = dayjs(session.check_in).format(
                          "MMM DD",
                        );
                        const isNextDay =
                          dayjs(session.check_in).format("YYYY-MM-DD") !==
                          log.date;

                        return (
                          <div
                            key={sIdx}
                            className={`border rounded-md p-3 ${hasMultipleSessions ? "bg-gray-50" : ""}`}
                          >
                            {/* Session Header */}
                            <div className="flex justify-between items-center mb-2 pb-2 border-b">
                              <Space>
                                ...
                                <Text strong>
                                  Session {session.session_number || sIdx + 1}
                                </Text>
                                {isNextDay && (
                              ...
                              <Tag color="blue">
                                Duration:{" "}
                                {Math.floor(
                                  session.duration?.total_minutes / 60,
                                )}
                                h {session.duration?.total_minutes % 60}m
                              </Tag>
                            </div>
                              ...
                              <div className="mt-2">
                                <div className="flex items-center gap-1 mb-1">
                                  ...
                                  <Text strong className="text-xs">
                                    Breaks ({session.break_periods.length})
                                  </Text>
                                </div>
                                <div className="flex flex-wrap gap-1">
                                  {session.break_periods.map((bp, i) => (
                                    <Tag
                                      ...
                                      className="text-xs"
                                    >
                                      {formatTime(bp.start_time)} →{" "}
                                      {formatTime(bp.end_time)} (
                            ...
                            <div className="mt-2 text-xs text-gray-500">
                              Worked: {session.duration?.worked_hours}h | Break:{" "}
                              {session.duration?.break_hours}h
                            </div>
                          </div>
                        );
                      })}
                    ...
                    <div className="mt-3 pt-2 border-t flex flex-wrap gap-1">
                      {log.summary?.is_holiday && (
                        <Tag color="gold">🏖️ Holiday</Tag>
                      )}
                      {log.summary?.has_paid_leave && (
                        <Tag color="green">✅ Paid Leave</Tag>
                      )}
                      {log.summary?.has_unpaid_leave && (
                        <Tag color="orange">⚠️ Unpaid Leave</Tag>
                      )}
                      {log.summary?.total_sessions > 1 && (
                        <Tag color="purple">
                          Total Sessions: {log.summary.total_sessions}
                        </Tag>
                      )}
                    </div>
                  </Card>
